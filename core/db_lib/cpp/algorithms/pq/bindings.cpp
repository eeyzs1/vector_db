...
PYBIND11_MODULE(vectordb_pq, m) {
    ...
    py::class_<IndexPQ>(m, "IndexPQ")
        .def(py::init<size_t, size_t, size_t>(),
             py::arg("dimension"), py::arg("M") = 8,
             py::arg("nbits") = 8)
        .def("train", [](IndexPQ &self, py::array_t<float> x) {
        ...
        .def("add", [](IndexPQ &self, py::array_t<float> x) {
            py::buffer_info buf = x.request();
            if (buf.ndim != 2) {
                throw std::invalid_argument("Input must be 2D array");
            }
        ...
        .def("search", [](IndexPQ &self, py::array_t<float> x, size_t k) {
            py::buffer_info buf = x.request();
            if (buf.ndim != 2) {
                throw std::invalid_argument("Input must be 2D array");
            }
        ...
        .def_property_readonly("ntotal", &IndexPQ::get_ntotal)
        .def_property_readonly("dimension", &IndexPQ::get_dimension)
        .def_property_readonly("M", &IndexPQ::get_M)
        .def_property_readonly("nbits", &IndexPQ::get_nbits);
}
