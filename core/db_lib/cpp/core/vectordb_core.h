...
// AVX2-only L2 distance computation. Use this in hot paths where AVX-512
// causes CPU frequency downclocking that hurts overall throughput
// (e.g., HNSW search with frequent small distance computations).
inline float compute_l2_distance_avx2(const float* query, const float* vec, size_t dim) {
    float dist = 0.0f;
...
public:
    VectorStorage(size_t dimension)
        ...
        // No reserve — let add() allocate exactly what's needed.
        // Reserve causes page-fault overhead during add for small datasets,
        // making us slower than FAISS (which doesn't reserve).
    }
