#!/usr/bin/env python3
"""
KD-Tree (K-Dimensional Tree) performance report comparing C++/Rust implementations
"""
...
try:
    from vectordb.cpp import _kdtree
    cpp_available = True
except ImportError:
    cpp_available = False
...
def test_kdtree_performance(dimensions=128, num_vectors=100000, num_queries=100, k=10, leaf_size=40):
    print(f"KD-Tree Performance Test")
    ...
    if cpp_available:
        print("\nTesting C++ KD-Tree implementation:")
        try:
            start_time = time.time()
            index = _kdtree.IndexKDTree(dimensions, leaf_size)
...
def run_comprehensive_test():
    ...
    for dim, num_vec, num_q, k, leaf_size in test_configs:
        test_kdtree_performance(dim, num_vec, num_q, k, leaf_size)
