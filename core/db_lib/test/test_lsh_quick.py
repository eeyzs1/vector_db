#!/usr/bin/env python3
"""Quick LSH test: speed + recall vs FAISS."""
import time
import numpy as np
import sys, os
...
import faiss
from vectordb.cpp import _lsh, _flat
...
def test_lsh(dim, n, num_tables, num_funcs):
    k = 10
    np.random.seed(42)
    ...
    # FAISS LSH (nbits = num_tables * num_funcs)
    ...
    print(f"  {dim:>6} {n:>8} {num_tables:>6} {num_funcs:>6} | "
          f"{format_time(add_cpp):>10} {format_time(add_faiss):>10} {add_spd:>6.2f}x | "
          f"{format_time(search_cpp):>10} {format_time(search_faiss):>12} {search_spd:>6.2f}x | "
          f"{recall_cpp:>5.1%} {recall_faiss:>6.1%}")


if __name__ == '__main__':
    print(f"  {'Dim':>6} {'N':>8} {'Tables':>6} {'Funcs':>6} | "
          f"{'Add C++':>10} {'Add FAISS':>10} {'Ratio':>7} | "
          f"{'Search C++':>10} {'Search FAISS':>12} {'Ratio':>7} | "
          f"{'R@C++':>6} {'R@FAISS':>7}")
    print('-' * 100)
    for cfg in [(64, 10000, 8, 4), (128, 50000, 8, 4)]:
        test_lsh(*cfg)
